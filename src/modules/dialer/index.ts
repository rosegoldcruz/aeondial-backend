...
import { markDispositioned, processDialerAmdResult, findCallByChannelId } from './orchestrator';
...
function parseUrlEncodedBody(payload: string): Record<string, string> {
...
function normalizeAmdCallbackPayload(req: FastifyRequest): {
  callId?: string;
  orgId?: string;
  amdStatus?: string;
  amdCause?: string;
...
export const dialerModule: FastifyPluginAsync = async (app) => {
  ...
  app.post('/calls/:call_id/amd_result', async (req, reply) => {
    ...
    if (!callId) return reply.status(400).send({ error: 'call_id is required' });
    ...
    try {
      await recordAmdResult({ call_id: callId, org_id: orgId, result: amdResult, cause, duration_ms: durationMs });
    } catch (err) {
    ...
    try {
      await processDialerAmdResult(callId, orgId, amdResult, cause, durationMs);
    } catch (err) {
      const msg = err instanceof Error ? err.message : 'Dialer orchestration error';
      logger.error({ err, org_id: orgId, call_id: callId }, 'Failed processing AMD result');
      return reply.status(500).send({ error: msg });
    }
  ...
  app.post('/call-attempts/:id/wrap-up', async (req, reply) => {
    ...
    if (attemptErr) return reply.status(500).send({ error: attemptErr.message });
    if (!attempt) return reply.status(404).send({ error: 'Call attempt not found' });

    // Call the atomic wrap-up function
    const { data, error } = await supabase.rpc('apply_dialer_wrap_up', {
    ...
    if (attemptRow?.call_id) {
      await markDispositioned(attemptRow.call_id, orgId);
    }
  ...
  app.post('/leads/:lead_id/notes', async (req, reply) => {
    ...
    const { lead_id } = req.params as { lead_id: string };
    const body = (req.body || {}) as {
      body?: string;
      note_type?: string;
  ...
  app.get('/call-attempts/:id', async (req, reply) => {
    ...
    if (error) return reply.status(500).send({ error: error.message });
    if (!data) return reply.status(404).send({ error: 'Call attempt not found' });
    return reply.send(data);
  });
  ...
  app.post('/agents/self/hangup', async (req, reply) => {
    ...
    let leadChannelId: string | null = null;
    let agentChannelId: string | null = null;

    if (callId) {
      const call = await findCallByChannelId(callId);
      if (call) {
    ...
    for (const chId of [agentChannelId, leadChannelId]) {
      if (!chId) continue;
      try {
        await ARI.channels.hangup(chId);
        hungUp.push(chId);
      } catch {
